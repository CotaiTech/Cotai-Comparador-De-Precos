...
type Seed = {
  ...
  originalPrices?: Partial<Record<StoreKey, number>>;
  imageUrl?: string;
};

const updatedAt = "2026-08-22T14:32:00.000Z";
const stores = ["escola", "amantino", "bh", "bahamas"] as const;
...
function buildProduct(store: StoreKey, seed: Seed): Product {
  const price = seed.prices[store];
  const originalPrice = seed.originalPrices?.[store];

  return {
    id: `${store}-mock-${seed.key}`,
    store,
    name: seed.name,
    normalizedName: normalizeProductName(seed.name),
    brand: seed.brand,
    quantity: seed.quantity,
    unit: seed.unit,
    packageText: seed.packageText,
    price,
    originalPrice,
    discountPercentage:
      originalPrice && originalPrice > price
        ...
        : undefined,
    promotion: Boolean(originalPrice && originalPrice > price),
    available: true,
...
export const mockProducts: Product[] = seeds.flatMap((seed) =>
  stores.map((store) => buildProduct(store, seed))
);
