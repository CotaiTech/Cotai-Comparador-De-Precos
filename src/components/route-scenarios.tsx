...
export function RouteScenarios({
  result,
  profile,
  selectedOptionId,
  onSelect,
  ...
  onSelect?: (selection: PurchaseOptionSelection) => void;
}) {
    ...
    <section className="rounded-[32px] border border-black/5 bg-white p-6 shadow-[0_18px_45px_rgba(16,34,21,0.06)]">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        {scenarios.map((scenario) => {
          ...
          const optionId = `route-${scenario.id}` as const;
          ...
          return (
            <button
              ...
              onClick={() =>
                onSelect?.({
                  ...
                  total: scenario.adjustedTotal,
                  purchaseCost: scenario.purchaseCost,
                  travelCost: scenario.travelCost,
            ...
            >
              ...
              <p className="mt-1 text-xs text-slate-500">Compra {formatCurrency(scenario.purchaseCost)} + trajeto {formatCurrency(scenario.travelCost)}</p>
              <div className="mt-4 flex flex-wrap gap-2 text-xs">
                <span className="rounded-full bg-white px-3 py-1.5">{scenario.distance.toFixed(1)} km</span>
                <span className="rounded-full bg-white px-3 py-1.5">~{scenario.estimatedMinutes} min</span>
                <span className="rounded-full bg-white px-3 py-1.5"><Fuel className="mr-1 inline h-3 w-3" />{scenario.stores.map((store) => storeMeta[store].label).join(" + ") || "Sem rota"}</span>
              </div>
              {selected ? <p className="mt-4 text-xs font-semibold text-slate-950">Selecionado para salvar</p> : null}
            </button>
          );
        })}
      </div>
